...
class EventInfoViewSetTest(APITestCase):
    ...
    def test_unauthenticated_event_edit_or_delete(self):
        self.client.post(reverse('user_info-list'), self.user_data, format='json')
        login_response = self.client.post(reverse('token-login'), self.user_data, format='json')
        self.client.cookies.update(login_response.cookies)
        ...
        update_response = self.client.patch(event_url, {'title': 'Should Fail'}, format='json')
